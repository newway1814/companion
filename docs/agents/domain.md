...
For UI work, the source of truth is the impeccable-managed `DESIGN.md` + `PRODUCT.md` at the repo root plus the design tokens in `src/app/globals.css` ([ADR 0004](../adr/0004-adopt-impeccable-design-md-as-ui-source-of-truth.md), superseding [ADR 0003](../adr/0003-adopt-in-repo-design-system.md)). Design new screens with the impeccable skill (`/impeccable …`) against `DESIGN.md` and reuse the shared tokens/components; do not revive the rejected Codex-generated prototype UI. The older `docs/design/design-system.md` is a superseded pointer, and the Stitch mockups under `docs/design/stitch/pages/` are historical MVP reference only — do not cite either to justify keeping or changing a design.
